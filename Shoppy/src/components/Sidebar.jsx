import { useState } from "react";
import { NavLink } from "react-router-dom";
import {
  FaShoppingBag,
  FaShoppingCart,
  FaUsers,
  FaUser,
  FaCalendar,
  FaTasks,
  FaEdit,
  FaPalette,
  FaChartLine,
  FaChartArea,
  FaChartBar,
  FaChartPie,
  FaMoneyBill,
  FaTh,
  FaSitemap,
  FaLayerGroup,
} from "react-icons/fa";

export default function Sidebar() {
  const [open, setOpen] = useState(true);

  const navData = [
    {
      section: "Dashboard",
      links: [{ name: "Ecommerce", href: "/", icon: <FaShoppingBag /> }],
    },
    {
      section: "Pages",
      links: [
        { name: "Orders", href: "/orders", icon: <FaShoppingCart /> },
        { name: "Employees", href: "/employees", icon: <FaUser /> },
        { name: "Customers", href: "/customers", icon: <FaUsers /> },
      ],
    },
    {
      section: "Apps",
      links: [
        { name: "Calendar", href: "/calendar", icon: <FaCalendar /> },
        { name: "Kanban", href: "/kanban", icon: <FaTasks /> },
        { name: "Editor", href: "/editor", icon: <FaEdit /> },
        { name: "Color Picker", href: "/colorpicker", icon: <FaPalette /> },
      ],
    },
    {
      section: "Charts",
      links: [
        { name: "Line", href: "/line", icon: <FaChartLine /> },
        { name: "Area", href: "/area", icon: <FaChartArea /> },
        { name: "Bar", href: "/bar", icon: <FaChartBar /> },
        { name: "Pie", href: "/pie", icon: <FaChartPie /> },
        { name: "Financial", href: "/financial", icon: <FaMoneyBill /> },
        { name: "Color Mapping", href: "/colormapping", icon: <FaTh /> },
        { name: "Pyramid", href: "/pyramid", icon: <FaSitemap /> },
        { name: "Stacked", href: "/stacked", icon: <FaLayerGroup /> },
      ],
    },
  ];

  return (
    <div className="flex">
      {/* Sidebar */}
      <div
        className={`fixed top-0 left-0 h-full bg-white ps-7 pe-2 py-5 shadow-lg overflow-auto w-72`}
      >
        {/* Logo */}
        <h2 className="text-xl font-bold mb-10 text-black flex items-center">
          <svg
            className="me-3 text-primary"
            stroke="currentColor"
            fill="currentColor"
            stroke-width="0"
            role="img"
            viewBox="0 0 24 24"
            height="1em"
            width="1em"
            xmlns="http://www.w3.org/2000/svg"
          >
            <title></title>
            <path d="M23.9477 10.8913a.1735.1735 0 00-.061-.1178c-2.5032-2.078-4.5288-2.9261-6.9905-2.9261-1.3127 0-2.32.2638-2.9916.7827-.5822.4492-.8896 1.0772-.8896 1.812 0 2.0605 2.5184 3.0003 5.4358 4.0883 1.5023.5604 3.057 1.1404 4.483 1.9319a.1626.1626 0 00.0828.0218.187.187 0 00.0589-.011c.0458-.0174.085-.0523.1025-.1002.545-1.3955.822-2.8673.822-4.374a13.082 13.082 0 00-.0523-1.1076zm-4.81 10.4791c-1.0423-.785-2.5795-1.3824-4.2061-2.0125-1.9362-.7501-4.132-1.6027-5.7803-2.913-1.8665-1.4871-2.7757-3.3623-2.7757-5.7324 0-2.1281.883-3.9466 2.5533-5.2614 1.873-1.474 4.7119-2.2546 8.2071-2.2546.966 0 1.8883.0589 2.743.1766a.1696.1696 0 00.1788-.098.17.17 0 00-.0414-.2007C17.814 1.0924 14.9664.0022 12.001.0022c-3.2052 0-6.2186 1.2472-8.4862 3.5148C1.2494 5.7825 0 8.796 0 11.999c0 3.2051 1.2472 6.2185 3.5149 8.484 2.2654 2.2654 5.2788 3.5148 8.4862 3.5148 2.5903 0 5.0564-.8133 7.1344-2.3505a.1714.1714 0 00.0697-.1374.1735.1735 0 00-.0676-.1395Z"></path>
          </svg>
          Shoppy
        </h2>

        {/* Nav Sections */}
        <nav className="space-y-6">
          {navData.map((group, i) => (
            <div key={i}>
              <p className="text-gray-400 uppercase text-md mb-3">
                {group.section}
              </p>
              {group.links.map((link, j) => (
                <NavLink
                  key={j}
                  to={link.href}
                  className={({ isActive }) =>
                    `text-md flex items-center rounded-lg px-4 py-3 ${
                      isActive
                        ? "text-white bg-[rgb(3,201,215)]"
                        : "text-gray-700 bg-white"
                    }`
                  }
                >
                  <span className="me-6">{link.icon}</span>
                  {link.name}
                </NavLink>
              ))}
            </div>
          ))}
        </nav>
      </div>

      {/* Toggle button */}
      <button
        onClick={() => setOpen(!open)}
        className="m-4 ms-72 p-2 primary-text"
      >
        <svg
          className="h-6 w-6"
          xmlns="http://www.w3.org/2000/svg"
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
        >
          <path
            strokeLinecap="round"
            strokeLinejoin="round"
            strokeWidth="2"
            d="M4 6h16M4 12h16M4 18h16"
          />
        </svg>
      </button>
    </div>
  );
}
